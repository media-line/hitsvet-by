...
jQuery(window).scroll(function(){
	if(jQuery('#pages_current').val() < jQuery('#pages_total').val())	{
		if(((jQuery(window).scrollTop()+jQuery(window).height())+150)>=jQuery(document).height()){
			if(loading == false){
				loading = true;
				jQuery('#loadingbar').show();
				jQuery.get('".$this->document->base."?start='+jQuery('#start').val()+'&format=raw$instock', function(loaded){
					jQuery('.pagination_wr').remove();
					jQuery('.c_part_WR').append(loaded);
					//jQuery('#loaded_max').val(parseInt($('#loaded_max').val())+50);
...
if($format == 'html')	{
	...
	<div id="loadingbar_wr"><img  id="loadingbar" src="/images/system/uploadVK.gif" /></div>
	</div>
	</div>
	</div>
