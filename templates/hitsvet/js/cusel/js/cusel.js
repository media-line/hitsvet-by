...
function cuSel(params) {
	...
	*/
	...
	{
		var pattern = /(val=)(.*?)(>)/g;
		spanStr = spanStr.replace(pattern, "jQuery1'jQuery2'jQuery3");
	}
...
function(event)
{
	...
	*/
	if(key==40)
	{
		...
		if(cuselActiveNext.is("span"))
		{
			...
			jQuery(this).find("input").eq(0).val(cuselActiveNext.attr("val"));
	...
	*/
	if(key==13)
	{
		...
		else var cuselHoverVal = jQuery(this).find(".cuselActive").attr("val");
	...
	*/
	...
	{
		var cuselWrap = jQuery(this).find(".cusel-scroll-wrap").eq(0);
...
jQuery(".cusel").keypress(function(event)
{
	...
	function handlingEvent()
	{
		...
		for(i=0;i<colArrOption;i++)
		{
			...
			/* прокручиваем к текущему оптиону */
			cuselScrollToCurent(jQuery(o).find(".cusel-scroll-wrap").eq(0));
			arr = arr.splice;
			arr = [];
			break;
			return true;
