...
// 		$db = JFactory::getDBO();
	...
	public function getManufacturersFromIds($ids = '') 
	{
		...
		$query->select(array('virtuemart_manufacturer_id', 'mf_name'));
		$query->from('#__virtuemart_manufacturers_'.VMLANG);
		$query->where("virtuemart_manufacturer_id IN ($ids)");
		$db->setQuery($query);
		$results = $db->loadObjectList("virtuemart_manufacturer_id");
		return $results;
	}
	//--------------------------------------
