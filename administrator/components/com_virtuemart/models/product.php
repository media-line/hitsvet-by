...
class VirtueMartModelProduct extends VmModel {
	...
	function updateRequests () {
		...
		//-----------------------------------
		$this->ProductInStock = JRequest::getVar ('instock', 0);
		//-----------------------------------
...
// 		vmdebug('my $this->searchcustoms ',$this->searchcustoms);
			...
			*/
			//-------------------------------------------------
			//echo'<pre>';print_r($this->searchcustoms);echo'</pre>';
			$searchcustom_num = 0;
			if (!empty($this->searchcustoms)) {
				$joinCustom = TRUE;
				$searchcustom_num = 1;
				$searchcustom_tables = array();
				$searchcustom_virtuemart_product_id = array();
				foreach ($this->searchcustoms as $key => $searchcustom) {
					$custom_search_str = '';
					foreach($searchcustom as $searchcustom1)	{
						$custom_search_str .= ' pf'.$searchcustom_num.'.`custom_value` like "%' . $this->_db->getEscaped ($searchcustom1, TRUE) . '%" OR';
					}
					$custom_search_str = '('.substr($custom_search_str, 0, -2).')';
					$custom_search[] = '(pf'.$searchcustom_num.'.`virtuemart_custom_id`="' . (int)$key . '" and '.$custom_search_str.')';
					$searchcustom_num++;
				}
				//echo'<pre>';print_r($this->searchcustoms);echo'</pre>';
				//echo'<pre>';print_r($searchcustom_tables);echo'</pre>';
				//echo'<pre>';print_r(implode(' AND ', $searchcustom_virtuemart_product_id));echo'</pre>';
				//echo'<pre>';print_r($custom_search);echo'</pre>';
				$where[] = " ( " . implode (' AND ', $custom_search) . " ) ";
				//$where[] = " ( " . implode (' OR ', $custom_search) . " ) ";
			}
			if($this->ProductInStock == 1)	{
...
// 			$joinLang = false;
		...
		if ($this->searchcustoms) {
			for ($i = 1; $i < $searchcustom_num; $i++) {
				$joinedTables[] = ' LEFT JOIN `#__virtuemart_product_customfields` as pf'.$i.' ON p.`virtuemart_product_id` = pf'.$i.'.`virtuemart_product_id` ';
			}		
//			$joinedTables[] = ' LEFT JOIN `#__virtuemart_product_customfields` as pf1 ON p.`virtuemart_product_id` = pf1.`virtuemart_product_id` ';
//			$joinedTables[] = ' LEFT JOIN `#__virtuemart_product_customfields` as pf2 ON p.`virtuemart_product_id` = pf2.`virtuemart_product_id` ';
		}
		if ($this->searchplugin !== 0) {
		...
		//echo'<pre>';print_r($whereString);echo'</pre>';
		//echo'<pre>';print_r($joinedTables);echo'</pre>';
	...
	public function setPaginationLimits () {
		...
		//--------------------------------- РїРѕСЃРєРѕР»СЊРєСѓ СЂРµР°Р»РёР·РѕРІР°РЅР° РїР°РіРёРЅР°С†РёСЏ Р°СЏРєСЃРѕРј С‚Рѕ РЅСѓР¶РЅРѕ СЃР±СЂР°СЃС‹РІР°С‚СЊ Р·РЅР°С‡РµРЅРёРµ СЃС‚Р°СЂС‚РѕРІРѕР№ РїРѕР·РёС†РёРё СЃРїРёСЃРєР° С‚РѕРІР°СЂРѕРІ.
		$limitstart_ = JRequest::getInt ('limitstart', -1);
		$limit_ = JRequest::getInt ('limit', -1);
		if($app->isSite() == true && $limitstart_ == -1 && $limit_ = -1)	{
			$limitStart	= 0;
		}
		//echo'<pre>';print_r($limitstart_);echo'</pre>';
		//echo'<pre>';print_r($limit_);echo'</pre>';
		//echo'<pre>';print_r($limitStart);echo'</pre>';
		//echo'<pre>';var_dump($app->isSite ());echo'</pre>';
		//$isSite = $;
		//---------------------------------
...
//	 		$q .= ' WHERE (`product_parent_id` = "'.$this->_id.'" AND (pc.`virtuemart_category_id`) IS NULL  ) OR (`virtuemart_product_id` = "'.$this->_id.'" ) ';
 ...
 //			vmdebug('getUncategorizedChildren '.$this->_db->getQuery(),$this->_uncategorizedChildren);
	...
	public function getProductsFromSeries ($virtuemart_product_id, $product_sku) {
		//echo'<pre>';print_r($product_sku);echo'</pre>';
		//$product_sku = 444;
		$product_sku_arr = explode('-', $product_sku);
		$like_product_sku = $product_sku_arr[0].'-';
		//echo'<pre>';print_r($product_sku_arr);echo'</pre>';
		if(count($product_sku_arr) != 2) return array();
		$query = "SELECT `virtuemart_product_id` FROM `#__virtuemart_products` WHERE `product_sku` like '$like_product_sku%' AND `virtuemart_product_id` <> $virtuemart_product_id";
		$this->_db->setQuery ($query);
	...
	//ищем поисковую фразу в пользовательских полях
	public function search_IdsInCustomFields ($search) {
		if($search != null)	{
			
			$query = "SELECT `virtuemart_product_id` FROM `#__virtuemart_product_customfields` WHERE `custom_value` like '%$search%'
						UNION
						SELECT `virtuemart_product_id` FROM `#__virtuemart_products` WHERE `product_sku` like '%$search%'";
	...
	//ищем поисковую фразу в артикуле
	public function search_IdsInProdSKU ($search) {
		if($search != null)	{
			$query = "SELECT `virtuemart_product_id` FROM `#__virtuemart_products` WHERE `product_sku` like '%$search%'";
			$this->_db->setQuery ($query);
			$ids = $this->_db->loadResultArray();
		}	else	{
			$ids = array();
