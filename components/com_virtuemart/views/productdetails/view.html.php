...
//		$limitstart	= JRequest::getVar('limitstart', 0, '', 'int');
	...
	//----------------------------- Получаем товары из этой же серии (первая часть артикула совпадает) -------
	$RelatedIds = array();
	foreach($product->customfieldsRelatedProducts as $rel)	{
		$RelatedIds[] = $rel->custom_value;
	}
	//echo'<pre>';print_r($RelatedIds);echo'</pre>';
	$ids = $product_model->getProductsFromSeries($product->virtuemart_product_id, $product->product_sku);
	$ids = array_unique(array_merge($ids, $RelatedIds));
	
	//$ids = $product_model->sortSearchListQuery (TRUE, $product->virtuemart_category_id, 'random', 4);
	//echo'<pre>';print_r($product->virtuemart_product_id);echo'</pre>';
	if(count($ids) > 0)	{
		$products_from_cat = $product_model->getProducts ($ids);
	}	else	{
		$products_from_cat = array();
	}
	
	//$products = $productModel->getProductsInCategory($categoryId);
	$product_model->addImages($products_from_cat,1);
	$this->assignRef('products_from_cat', $products_from_cat);
	
	//echo'<pre>';print_r($ids);echo'</pre>';
	//echo'<pre>';print_r($products_from_cat);echo'</pre>';
	//------------------------------------------
