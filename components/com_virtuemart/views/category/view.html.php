	...
	public function display($tpl = null) {
		...
		if(!empty($category)){
			...
			if(empty($category->category_layout) or $category->category_layout != 'categories') {
				...
				//echo'<pre>';var_dump($search);echo'</pre>';
				if($search != null)	{
					//ищем поисковую фразу и пользовтельских полях
					$ids_search = $productModel->search_IdsInCustomFields($search);
					$ids = array_merge($ids,$ids_search);
					$ids = array_unique($ids); //поскольку могу возникнуть дубли позиций - убираем их
				...
				//-----------------------------------
				// проверяем наличие соотв. товаров в корзине
				if($search == null and count($products))	{
					$cart = VirtueMartCart::getCart();
					foreach($products as $i)	{
						if(isset($cart->products[$i->virtuemart_product_id]))	{
							$i->product_in_cart = 1;
						}	else	{
							$i->product_in_cart = 0;
						}
